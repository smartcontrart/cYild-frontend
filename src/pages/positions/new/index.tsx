...
export default function NewPositionPage() {
  ...
  const handleCreatePosition = async () => {
    ...
    if (userAddress) {
      ...
      if (selectedToken0 && selectedToken1 && accountingUnit) {
        ...
        if (
          selectedToken0.address.toLowerCase() !==
            accountingUnit.address.toLowerCase() &&
          existingPoolsFor0.length === 0
        )
  ...
  const onOpenPosition = async () => {
    ...
    const [token0SortedByCA, token1SortedByCA] =
      reArrangeTokensByContractAddress([selectedToken0, selectedToken1]);
    ...
    if (!walletClient) {
      ...
      toast({
        ...
        title: "Error",
        description:
          "Wallet client not available. Please ensure your wallet is properly connected.",
      });
      return;
    }
    ...
    try {
      ...
      const { success: approveToken0Success } = await approveToken(
        userAddress,
      ...
      const { success: approveToken1Success } = await approveToken(
        userAddress,
      ...
      // Add detailed logging before the openPosition call
      console.log("About to call openPosition with:", {
      ...
      const { success: openPositionSuccess, result } = await openPosition(
        publicClient,
        chainId,
        {
          token0Address: token0SortedByCA.address,
      ...
      if (!openPositionSuccess) {
        ...
        console.log("sortedToken0Amount", sortedToken0Amount);
        console.log("sortedToken1Amount", sortedToken1Amount);
        console.log("tickUpper", tickUpper);
        console.log("tickLower", tickLower);
        console.log("token0SortedByCA", token0SortedByCA);
        console.log("token1SortedByCA", token1SortedByCA);
        console.log("selectedToken0", selectedToken0);
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: result || "Failed to open position",
        });
      ...
      >
        <AlertDialogContent>
          ...
          <div>
            ...
            {currentAccountingUnit?.symbol}/{selectedToken0?.symbol} and{" "}
            {currentAccountingUnit?.symbol}/{selectedToken1?.symbol}. If any of
            those pools does not exist, your position will not be protected by
            Yild fail-safe functions.
          </div>
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => {
