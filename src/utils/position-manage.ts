...
export const compoundFees = async (
  ...
  let _pSwapData0 = "0x",
    _pSwapData1 = "0x",
    _token0MaxSlippage = userMaxSlippage,
    _token1MaxSlippage = userMaxSlippage,
    _minAmountOut0 = BigInt(0),
    _minAmountOut1 = BigInt(0);
  if (sell0For1) {
    const {
      success: paraswapAPISuccess,
      data: paraswapData,
      destAmount: amountOut0,
...
export const openPosition = async (
  ...
  try {
    console.log("Attempting to write contract with params:", params);

    // Use writeContract with walletClient
    const { request } = await publicClient.simulateContract({
      account: walletClient.account,
      address: getManagerContractAddressFromChainId(chainId),
      abi: PositionManagerABI,
    ...
    console.error("OpenPosition error details:", {
    ...
    return {
      success: false,
      result: error?.message || ERROR_CODES.UNKNOWN_ERROR,
    };
  }
};
