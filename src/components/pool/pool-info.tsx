...
export function PoolInfo({
  ...
  const { loading, error, data } = useQuery(GET_DATA, {
    client: getClientFromChainId(chainId),
  });
  ...
  return (
    <Card
      className={`p-6 cursor-pointer ${selected ? (theme === "dark" ? "bg-blue-600" : "bg-sky-100") : ""}`}
      key={`feeTier_${feeTier}`}
      onClick={() => onClickPool && onClickPool(feeTier)}
    >
