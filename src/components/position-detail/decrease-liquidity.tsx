...
export const DecreaseLiquidity = ({
  ...
  const { data: positionFundsInfo, isLoading: isPositionFundsInfoLoading } =
    usePositionFundsInfo(positionId, chainId);
  ...
  const onClickDecreaseLiquidity = async () => {
    if (!positionStaticInfo) return;
    const amountInBPS = Math.round(
      roundDown(parseFloat(decreaseRatio) * 100, 0)
    );
    try {
      setPageStatus(POSITION_DETAIL_PAGE_STATE.DECREASING_LIQUIDITY);
      const { success, result } = await decreaseLiquidity(
        positionId,
        chainId,
        amountInBPS,
        walletClient,
            ...
            <div className="flex flex-row gap-2 items-center">
              ...
              <div className="grid items-center gap-1">
                <Input
                  type="number"
                  className="w-24"
                  onChange={(e) => setDecreaseRatio(e.target.value)}
                  value={decreaseRatio}
                />
              </div>
              <div>% of the position.</div>
            </div>
            <div
              className={` flex-col gap-2 items-center ${
                Number(decreaseRatio) >= 100 || Number(decreaseRatio) < 1
                  ? "hidden"
                  : "flex"
              }`}
            >
              <div>
                {(
                  (Number(
                    formatUnits(
                      positionFundsInfo.principal0,
                      positionStaticInfo.token0?.decimals
                    )
                  ) *
                    Number(decreaseRatio)) /
                  100
                ).toFixed(6)}{" "}
              ...
              <div>
                ...
                {positionStaticInfo.token1.symbol}
              </div>
            </div>
            <DialogFooter>
              <Button
                disabled={
                  !decreaseRatio ||
                  !parseFloat(decreaseRatio) ||
                  parseFloat(decreaseRatio) < 0.1 ||
                  parseFloat(decreaseRatio) > 99
                }
                onClick={onClickDecreaseLiquidity}
              >
                <Check />
                Withdraw
              </Button>
