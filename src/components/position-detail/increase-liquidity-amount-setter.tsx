...
export const IncreaseLiquidityAmountSetter = ({
  ...
  const [increaseToken0Amount, setIncreaseToken0Amount] =
    useState(token0Amount);
  const [increaseToken1Amount, setIncreaseToken1Amount] =
    useState(token1Amount);
  const { data: token0Price } = useTokenPrice(token0.address, chainId);
  const { data: token1Price } = useTokenPrice(token1.address, chainId);
  const { data: token0Balance, isLoading: isToken0BalanceLoading } =
    useTokenBalance(userAddress || "", token0.address, chainId);
  const { data: token1Balance, isLoading: isToken1BalanceLoading } =
  ...
  useEffect(() => {
    ...
    if (isUserEditingForToken0) {
      ...
      const newToken0Amount = getRequiredToken0AmountFromToken1Amount(
        ...
        priceLower,
        priceUpper,
        Number(increaseToken1Amount)
      );
      const roundedToken0Amount = roundDown(newToken0Amount, token0.decimals);
      onValuesChange({
        token0Amount: formatForDisplay(roundedToken0Amount, token0.decimals),
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3 text-sm">
              Available:{" "}
              {formatUnits(BigInt(token0Balance || 0), token0.decimals)}
            </div>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="name" className="text-right">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3 text-sm">
              Available:{" "}
              {formatUnits(BigInt(token1Balance || 0), token1.decimals)}
            </div>
          </div>
        </div>
      )}
