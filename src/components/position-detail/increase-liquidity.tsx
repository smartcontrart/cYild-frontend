...
export const IncreaseLiquidity = ({
  ...
  const onClickIncreaseLiquidity = async () => {
    ...
    try {
      ...
      const { success: addLiquiditySuccess, result } = await increaseLiquidity(
        chainId,
        {
          tokenId: positionId,
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <IncreaseLiquidityAmountSetter
              ...
              onValuesChange={(data: any) => {
                if (typeof data.token0Amount === "string")
                  setIncreaseToken0Amount(data.token0Amount.toString());
                if (typeof data.token1Amount === "string")
                  setIncreaseToken1Amount(data.token1Amount.toString());
              }}
            />
            <DialogFooter>
              <Button
                disabled={
                  ...
                  Number(increaseToken0Amount) <= 0 ||
                  !Number(increaseToken1Amount) ||
                  Number(increaseToken1Amount) <= 0
                }
                onClick={() => onClickIncreaseLiquidity()}
              >
                <Check />
                Deposit
              </Button>
